...
TS_VERSION="3.13.7"
TS_ARCH="amd64"
TS_URL="https://files.teamspeak-services.com/releases/server/${TS_VERSION}/teamspeak3-server-linux-${TS_ARCH}-${TS_VERSION}.tar.bz2"
...
if id -u $TS_USER >/dev/null 2>&1; then
    ...
    adduser --system --group --no-create-home $TS_USER
fi
...
tar -xjf /tmp/teamspeak.tar.bz2 -C /tmp
mv /tmp/teamspeak3-server-linux-$TS_ARCH-$TS_VERSION/* $TS_INSTALL_DIR
rm -rf /tmp/teamspeak3-server-linux-$TS_ARCH-$TS_VERSION /tmp/teamspeak.tar.bz2

echo "Setare permisiuni..."
chown -R $TS_USER:$TS_USER $TS_INSTALL_DIR
chmod -R 755 $TS_INSTALL_DIR

echo "Creare director date..."
mkdir -p $TS_DATA_DIR
chown -R $TS_USER:$TS_USER $TS_DATA_DIR
...
echo "$TS_DATA_DIR/logs/ts3server_*.log"
